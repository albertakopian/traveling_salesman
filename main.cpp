...
void fill_value_of_path(const int&n, const int& num_steps, vector<vector<double>> &real_ans, vector<vector<double>> &match_ans, vector<vector<double>> &triangle_ans)
{
    for (int i = 2; i < n; ++i){
...
int main()
{
    ...
    fill_value_of_path(n, num_steps, real_ans, match_ans, triangle_ans);

    for (int i = 2; i < n; ++i)
    {
        cout << "for maching " << i << " : " << avg_square_deviation_quality(real_ans[i], match_ans[i], num_steps) << '\n';
        cout << "for triangle " << i << " : " << avg_square_deviation_quality(real_ans[i], triangle_ans[i], num_steps) << '\n';
    }
